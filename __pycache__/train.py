...
def main():
    ...
    if continue_training == True:
        ...
        strip_square_brackets(f"results/{current_model}train_loss_{run + 1}.txt")
        strip_square_brackets(f"results/{current_model}train_top1acc_{run + 1}.txt")
        strip_square_brackets(f"results/{current_model}train_top5acc_{run + 1}.txt")
            
        strip_square_brackets(f"results/{current_model}test_loss_{run + 1}.txt")
        strip_square_brackets(f"results/{current_model}test_top1acc_{run + 1}.txt")
        strip_square_brackets(f"results/{current_model}test_top5acc_{run + 1}.txt")

        train_loss_lst = list(genfromtxt(f"results/{current_model}train_loss_{run + 1}.txt", delimiter=','))
        train_top1acc_lst = list(genfromtxt(f"results/{current_model}train_top1acc_{run + 1}.txt", delimiter=','))
        train_top5acc_lst = list(genfromtxt(f"results/{current_model}train_top5acc_{run + 1}.txt", delimiter=','))

        test_loss_lst = list(genfromtxt(f"results/{current_model}test_loss_{run + 1}.txt", delimiter=','))
        test_top1acc_lst = list(genfromtxt(f"results/{current_model}test_top1acc_{run + 1}.txt", delimiter=','))
        test_top1acc_lst = list(genfromtxt(f"results/{current_model}test_top5acc_{run + 1}.txt", delimiter=','))

    for run in range(last_run, nruns):
        print(f'Run {run + 1}/{nruns}: Simple RNN initalised with {nlayers} layers and {hidden_size} number of hidden neurons.')
        ...
        for epoch in range(last_epoch, nepochs):
            ...
            print(f"Epoch:{epoch + last_epoch + 1}   Train[Loss:{train_loss_value} Top1 Acc:{train_top1acc_value}  Top5 Acc:{train_top5acc_value}]")
            print(f"Epoch:{epoch + last_epoch + 1}   Test[Loss:{test_loss_value}   Top1 Acc:{test_top1acc_value}   Top5 Acc:{test_top5acc_value}]")

            if save_model == True and ((epoch + last_epoch + 1) % 10) == 0 or epoch + last_epoch == nepochs - 1 :
                torch.save({
                ...
                with open(f'results/{current_model}train_loss_{run + 1}.txt','w') as values:
                    values.write(str(train_loss_lst))
                with open(f'results/{current_model}train_top1acc_{run + 1}.txt','w') as values:
                    values.write(str(train_top1acc_lst))
                with open(f'results/{current_model}train_top5acc_{run + 1}.txt','w') as values:
                    values.write(str(train_top5acc_lst))

                with open(f'results/{current_model}test_loss_{run + 1}.txt','w') as values:
                    values.write(str(test_loss_lst))
                with open(f'results/{current_model}test_top1acc_{run + 1}.txt','w') as values:
                    values.write(str(test_top1acc_lst))
                with open(f'results/{current_model}test_top5acc_{run + 1}.txt','w') as values:
